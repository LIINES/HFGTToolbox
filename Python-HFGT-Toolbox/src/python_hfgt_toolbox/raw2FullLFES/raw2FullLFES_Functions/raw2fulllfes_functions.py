...
import numpy as np
import pandas as pd
from scipy.sparse import (coo_matrix, csr_matrix, dok_matrix, find, hstack,
                          lil_matrix, vstack)

import sparse
from python_hfgt_toolbox.XML2LFES.XML2LFES_Classes import (Arc, PetriNetwork,
                                                           methodxForm,
                                                           methodxPort)
from sparse import COO, DOK, dot, kron
...
def calcMachineIdxForm(LFES):
    ...
    for k1 in range(len(LFES.machines.methodsxForm)):
        if isinstance(LFES.machines.methodsxForm[k1], methodxForm):
            for k2 in range(len(LFES.machines.methodsxForm[k1].name)):
                LFES.machines.methodsxForm[k1].idxForm.append(
                    LFES.setTransformProcess.index(LFES.machines.methodsxForm[k1].name[k2]))
    ...
    for k1 in range(len(LFES.machines.methodsxPort)):
        if isinstance(LFES.machines.methodsxPort[k1], methodxPort):
            for k2 in range(len(LFES.machines.methodsxPort[k1].origin)):
                LFES.machines.methodsxPort[k1].idxOrigin.append(
                    LFES.resources.name.index(LFES.machines.methodsxPort[k1].origin[k2]))
                LFES.machines.methodsxPort[k1].idxDest.append(
                    LFES.resources.name.index(LFES.machines.methodsxPort[k1].dest[k2]))
                LFES.machines.methodsxPort[k1].idxHold.append(
                    LFES.abstract.methodsxPort.ref.index(LFES.machines.methodsxPort[k1].ref[k2]))
                LFES.machines.methodsxPort[k1].idxPort.append(
                    getIdxPort(LFES.machines.methodsxPort[k1].idxOrigin[k2],
                               LFES.machines.methodsxPort[k1].idxDest[k2], LFES.numBuffers))
                LFES.machines.methodsxPort[k1].idxPortRef.append(
                    getIdxPortRef(LFES.machines.methodsxPort[k1].idxPort[k2],
                                  LFES.machines.methodsxPort[k1].idxHold[k2], LFES.numTransportProcess))
    ...
    for k1 in range(len(LFES.indBuffers.methodsxPort)):
        if isinstance(LFES.indBuffers.methodsxPort[k1], methodxPort):
            for k2 in range(len(LFES.indBuffers.methodsxPort[k1].origin)):
                LFES.indBuffers.methodsxPort[k1].idxOrigin.append(
                    LFES.resources.name.index(LFES.indBuffers.methodsxPort[k1].origin[k2]))
                LFES.indBuffers.methodsxPort[k1].idxDest.append(
                    LFES.resources.name.index(LFES.indBuffers.methodsxPort[k1].dest[k2]))
                LFES.indBuffers.methodsxPort[k1].idxHold.append(
                    LFES.abstract.methodsxPort.ref.index(LFES.indBuffers.methodsxPort[k1].ref[k2]))
                LFES.indBuffers.methodsxPort[k1].idxPort.append(
                    getIdxPort(LFES.indBuffers.methodsxPort[k1].idxOrigin[k2],
                               LFES.indBuffers.methodsxPort[k1].idxDest[k2], LFES.numBuffers))
                LFES.indBuffers.methodsxPort[k1].idxPortRef.append(
                    getIdxPortRef(LFES.indBuffers.methodsxPort[k1].idxPort[k2],
                                  LFES.indBuffers.methodsxPort[k1].idxHold[k2], LFES.numTransportProcess))
    ...
    for k1 in range(len(LFES.transporters.methodsxPort)):
        if isinstance(LFES.transporters.methodsxPort[k1], methodxPort):
            for k2 in range(len(LFES.transporters.methodsxPort[k1].origin)):
                LFES.transporters.methodsxPort[k1].idxOrigin.append(
                    LFES.resources.name.index(LFES.transporters.methodsxPort[k1].origin[k2]))
                LFES.transporters.methodsxPort[k1].idxDest.append(
                    LFES.resources.name.index(LFES.transporters.methodsxPort[k1].dest[k2]))
                LFES.transporters.methodsxPort[k1].idxHold.append(
                    LFES.abstract.methodsxPort.ref.index(LFES.transporters.methodsxPort[k1].ref[k2]))
                LFES.transporters.methodsxPort[k1].idxPort.append(
                    getIdxPort(LFES.transporters.methodsxPort[k1].idxOrigin[k2],
                               LFES.transporters.methodsxPort[k1].idxDest[k2], LFES.numBuffers))
                LFES.transporters.methodsxPort[k1].idxPortRef.append(
                    getIdxPortRef(LFES.transporters.methodsxPort[k1].idxPort[k2],
                                  LFES.transporters.methodsxPort[k1].idxHold[k2], LFES.numTransportProcess))
...
def calcJS_KS_AS(LFES):
    ...
    LFES.JS = vstack([hstack([LFES.JM, csr_matrix((LFES.numTransformProcess, LFES.numIndBuffers + LFES.numTransporters),
                                                  dtype=int)]), LFES.JHref], format='csr')
    LFES.KS = vstack([hstack([LFES.KM, csr_matrix((LFES.numTransformProcess, LFES.numIndBuffers + LFES.numTransporters),
                                                  dtype=int)]), LFES.KHref], format='csr')
    LFES.AS = vstack([hstack([LFES.AM, csr_matrix((LFES.numTransformProcess, LFES.numIndBuffers + LFES.numTransporters),
                                                  dtype=int)]), LFES.AHref], format='csr')
...
def initializeControl(LFES):
    ...
    LFES.SAMproj = coo_matrix((LFES.numControllers + LFES.DOFS, LFES.numControllers + LFES.DOFS), dtype=int)
    ...
    if LFES.numTransformProcess > 0:
        if LFES.numControlProcess > 0:
            for k1 in range(LFES.numTransformProcess):
                LFES.CPPAM[k1, LFES.numProcesses:LFES.numProcesses + LFES.numControlProcess] =\
                    [True if LFES.setTransformProcess[k1] in x else False for x in LFES.setControlProcess]
    if LFES.numHoldingProcess > 0:
        if LFES.numControlProcess > 0:
            for k1 in range(LFES.numHoldingProcess):
                LFES.CPPAM[LFES.numTransformProcess + (k1 * LFES.numTransportProcess):LFES.numTransformProcess + (k1 + 1) *
                           LFES.numTransportProcess, LFES.numProcesses:LFES.numProcesses + LFES.numControlProcess] = \
                    np.tile([True if LFES.setHoldingProcess[k1] in x else False for x in LFES.setControlProcess],
                            (LFES.numTransportProcess, 1))
    if LFES.numControlProcess > 0:
        for k1 in range(LFES.numControlProcess):
            if "store" in LFES.setControlProcess[k1]:
                storage = np.array([True if "store" in x else False for x in LFES.setTransportRefProcess]).reshape(
                    LFES.CPPAM[LFES.numTransformProcess:, LFES.numProcesses + k1].shape)
                LFES.CPPAM[LFES.numTransformProcess:, LFES.numProcesses + k1] = \
                    np.logical_and(LFES.CPPAM[LFES.numTransformProcess:, LFES.numProcesses + 3].toarray(), storage)
    ...
    for k1 in range(numControllers):
        if hasattr(LFES.controllers.peerRecipients[k1], 'name'):
            ...
            for k2 in range(numRecipients):
                idx = [True if item == LFES.controllers.peerRecipients[k1].name[k2] else
                       False for item in LFES.controllers.name]
...
def makeServiceNet(LFES):
    '''
    This generates a serviceNet object for each service and fills in the Mpos, Mneg, and dualAdjacency variables.
    :param LFES:
    :return:
    '''
...
def makeServiceFeasibility(LFES):
    ...
    xPortSingleLambda = np.zeros((0, LFES.numHoldingProcess))
    for k1 in range(LFES.numServices):
        print('Service %d' % (k1))
        linkName1 = LFES.services.serviceTransitions[k1].methodLinkName
        linkRef1 = LFES.services.serviceTransitions[k1].methodLinkRef
        ...
        xPortSingleLambda = np.vstack((xPortSingleLambda, LFES.services.xPortLambda[k1]))
    LFES.xPortLambda = xPortSingleLambda
...
def calcMRTneg(myLFES):
    ...
    MRT_neg = DOK((myLFES.numOperands, myLFES.numBuffers, myLFES.numProcesses * myLFES.numResources, 1), dtype=int)
    MRT_neg = COO(MRT_neg)
    MRTproj_neg = DOK((myLFES.numOperands, myLFES.numBuffers, myLFES.DOFS, 1), dtype=bool)
    MRTproj_neg = COO(MRTproj_neg)
    for i in range(myLFES.numOperands):
        for y2 in range(myLFES.numBuffers):
            ...
            # Add to MRT_pos
            MRT_neg += np.multiply.outer(e_iy2, Xiy1VP)
            # Add projection
            dof_projection = applyProjMat(Xiy1V, ASV)
            MRTproj_neg += np.multiply.outer(e_iy2, dof_projection)
...
def calcMRTpos(myLFES):
    ...
    for i in range(myLFES.numOperands):
        for y2 in range(myLFES.numBuffers):
            ...
            # Add to MRT_pos
            MRT_pos += np.multiply.outer(e_iy2, Xiy2VP)
            # Add projection
            dof_projection = applyProjMat(Xiy2V, ASV)
            MRTproj_pos += np.multiply.outer(e_iy2, dof_projection)
...
def calcARPar(LFES):
    ...
    # Clear folder of previous parallel parts
    files = glob.glob('data/ARPar/*.csv')
    for f in files:
        os.remove(f)
    print('in AR loop with %d processors' % (numProcessors))
    start_time = time.time()
    executor = ProcessPoolExecutor(max_workers=numProcessors)
    [executor.submit(calcARLoop, LFES, dataDiv[k1], dataDiv[k1 + 1], idxN, k, j, u, procIdx) for k1 in range(numProcessors)]
...
def calcARLoop(LFES, start_split, end_split, idxN, k, j, u, procIdx):
    ...
    for i1 in range(start_split, end_split):
        ...
        for i2 in range(len(idxN)):
            ...
            condition4 = (((((u1 - 1) % sBs) + 1 == math.floor((u2 - 1) / sBs) + 1 or (
                (u1 - 1) % sBs) + 1 == k2) and u1 > 0 and u2 > 0))
            if myMethods == 1:
                temp1 = [LFES.abstract.methodPair.idxProc1[k1] == procIdx1 for k1 in
                         range(len(LFES.abstract.methodPair.idxProc1))]
                temp2 = [LFES.abstract.methodPair.idxProc2[k1] == procIdx2 for k1 in
                         range(len(LFES.abstract.methodPair.idxProc1))]
    ...
    save_dir = Path('data/ARPar')
    save_dir.mkdir(parents=True, exist_ok=True)
    idxARfilename = ('%s/[pid.%s]_idxAR_calculation_on_range_starting_%s' % (save_dir, os.getpid(), start_split))
    DOFPairfilename = ('%s/[pid.%s]_DOFPair_calculation_on_range_starting_%s' % (save_dir, os.getpid(), start_split))
...
def importidxARandARprojPair(LFES):
    ...
    filenames = glob.glob('data/ARPar/*idxAR' + "*.csv")
    dfs = []
    for filename in filenames:
        dfs.append(pd.read_csv(filename))
    LFES.idxAR = pd.concat(dfs, ignore_index=True).to_numpy()
    filenames = glob.glob('data/ARPar/*DOFPair' + "*.csv")
    dfs = []
    for filename in filenames:
        dfs.append(pd.read_csv(filename))
...
def calcARInc(LFES):
    ...
    AR = COO(dot(sMR_pos.transpose(), sMR_neg))
    LFES.AR = AR
    ...
    LFES.ARproj = csr_matrix((np.ones(len(LFES.idxARproj[:, 0])), (LFES.idxARproj[:, 0],
                                                                   LFES.idxARproj[:, 1])), shape=(LFES.DOFS, LFES.DOFS))
...
def calcARIncJulia(LFES):
    ...
    # export mats to CSV
    save_dir = Path("data/ARJulia/")
    save_dir.mkdir(parents=True, exist_ok=True)
    np.savetxt(f"{save_dir}/MR_neg.csv", np.transpose(MR_neg.nonzero()), delimiter=",", header="buffer,DOF", comments='')
    np.savetxt(f"{save_dir}/MR_pos.csv", np.transpose(MR_pos.nonzero()), delimiter=",", header="buffer,DOF", comments='')

    os.system("julia calcARJulia.jl")

    LFES.idxAR = pd.read_csv(f"{save_dir}/ARIdx.csv")
...
def combineHFAMandCADM(LFES):
    ...
    LFES.SAMproj = SystemAdjacencyMatrix.tocsr()
