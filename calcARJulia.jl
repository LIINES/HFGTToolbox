using Pkg
Pkg.activate(".") # load environment from Project.toml
...
function run_loop(MR_neg_df,MR_pos_df)
    ...
    CSV.write("data/ARJulia/ARIdx.csv",  Tables.table(dofsUni), writeheader=false)
end


println("Loading CSVs")
MR_neg_df = CSV.read("data/ARJulia/MR_neg.csv", DataFrame)
MR_pos_df = CSV.read("data/ARJulia/MR_pos.csv", DataFrame)
